 ...
 * <ul>
 *   <li>GET  /api/webhooks/whatsapp (e /verify) — verification handshake</li>
...
public class WhatsappWebhookController {
     ...
     * Suporta a raiz "/api/webhooks/whatsapp" e o legado "/api/webhooks/whatsapp/verify".
     */
    ...
    public ResponseEntity<String> verify(
        ...
        log.info("WhatsApp webhook GET recebido para verificação: mode={}", mode);
 
        if (!enabled) {
            log.warn("WhatsApp está desabilitado (WHATSAPP_ENABLED=false)");
            return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).build();
        }
        if ("subscribe".equals(mode) && verifyToken.equals(token)) {
            log.info("WhatsApp webhook verificado com sucesso. Token de verificação válido.");
            return ResponseEntity.ok(challenge);
        }
 
        log.warn("Tentativa de verificação do WhatsApp inválida: mode={}, token não confere", mode);
    ...
    public ResponseEntity<Void> receberWebhook(
        ...
        log.info("WhatsApp webhook POST recebido da Meta.");
        ...
        if (!validarAssinatura(rawBody, signature)) {
            log.warn("Assinatura X-Hub-Signature-256 inválida ou ausente — payload rejeitado");
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
 
        log.info("Assinatura X-Hub-Signature-256 validada com sucesso.");
